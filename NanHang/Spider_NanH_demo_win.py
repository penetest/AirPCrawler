...
def data_analyze(loaddata,filename):
    ...
    #loaddata=json.loads( loaddata)
    airports_o_code = loaddata['citys'][0]['code']
    airports_o_cityname = loaddata['citys'][0]['zhName']
    airports_d_code = loaddata['citys'][1]['code']
    airports_d_cityname = loaddata['citys'][1]['zhName']
    #print airports_d_cityname
    dataflight = loaddata['segment']['dateflight']['flight']
    for data in dataflight:
        airline = data['airline']
        flightno = data['flightno']
        plane = data['plane']
        timeduringflight=data['timeduringflight']
        arrdate=str(data['arrdate'])+str(data['arrtime'])
        depdate=str(data['depdate'])+str(data['deptime'])
        cabin = data['cabin']
        for price in cabin:
            name = price['name']
            adultprice = price['adultprice']
            discount=price['discount']
            info2 = airline+','+flightno+','+airports_o_cityname+','+airports_o_code+','+depdate+','+airports_d_cityname+','+airports_d_code+','+arrdate+','+plane+','+timeduringflight+','+name+','+adultprice+','+discount
...
def swap_o_d(i_dstCity, i_orgcity, i_startDate):
    ...
    filename = "NanH-"+currdatatime+"-"+ startDate + ".csv"
    ...
    for n in range(9):
        ...
        try:
            loaddata = json.loads(loaddata)
            flag = str(loaddata['segment'])
        except ValueError:
            print loaddata
            print '页面抓取失败..,尝试重新抓取1'.encode("GBK")
            aa = Adsl()
            aa.reconnect()
            time.sleep(5)
            continue
            ...
            #print loaddata
            print '页面抓取失败..,尝试重新抓取11'.encode("GBK")
            aa = Adsl()
            aa.reconnect()
            time.sleep(5)
            continue
            ...
            try:
                flag1 = str(loaddata['message'])
                if(len(flag1) > 0):
                    return 0
            except KeyError:
                print '页面抓取失败..,尝试重新抓取12'.encode("GBK")
                aa = Adsl()
                aa.reconnect()
                time.sleep(5)
                continue
...
if __name__ == "__main__":
    main()
    # ( loaddata,code)= data_Crawling('PEK','CAN','2016-07-28')
    # print loaddata
    # with open("NanHang.json",'w') as fp:
    #     json.dump(loaddata,fp)
    # with open("NanHang.json",'r') as fpr:
    #     loaddata = json.load(fpr)
    # data_analyze(loaddata,"Nanh.csv")
