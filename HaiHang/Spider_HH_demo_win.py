...
def data_Crawling(from_h,to_h,date_h,SC):
    s = requests.Session()
    try:
        resp1 = s.get('http://new.hnair.com/hainanair/ibe/deeplink/ancillary.do?DD1='+date_h+'&DD2=&TA=1&TC=0&TI=&ORI='+from_h+'&DES='+to_h+'&SC='+SC+'&ICS=F&PT=F&PT=&FLC=1&NOR=&PACK=T',timeout=15)

        resp2 = s.post('http://new.hnair.com/hainanair/ibe/deeplink/ancillary.do?DD1='+date_h+'&DD2=&TA=1&TC=0&TI=&ORI='+from_h+'&DES='+to_h+'&SC='+SC+'&ICS=F&PT=F&PT=&FLC=1&NOR=&PACK=T&redirected=true',timeout=15)

        resp3 = s.get('http://new.hnair.com/hainanair/ibe/deeplink/entryPointRedirect.do?QUERY=ancillaryIBESearch',timeout=15)

        #resp4 = s.post('http://new.hnair.com/hainanair/ibe/deeplink/entryPointRedirect.do?redirected=true')

        resp5 = s.get('http://new.hnair.com/hainanair/ibe/common/processSearchEntry.do?fromEntryPoint=true',timeout=15)

        resp6 = s.get('http://new.hnair.com/hainanair/ibe/common/spinner.do',timeout=15)

        #respwait = s.get('http://www.hnair.com/qt/dkggpic/wait/')

        resp7 = s.post('http://new.hnair.com/hainanair/ibe/common/processSearch.do',timeout=15)

        resp8 = s.get('http://new.hnair.com/hainanair/ibe/air/processSearch.do',timeout=15)

        resp = s.get('http://new.hnair.com/hainanair/ibe/air/searchResults.do',timeout=15)
